...
void main() {
  ...
  testWidgets(
    ...
    when(mockNotifier.tvSeason).thenReturn(testTvSeason);
  ...
  testWidgets(
      'Watchlist button should display Snackbar when TV Series added to watchlist',
      (WidgetTester tester) async {
    when(mockNotifier.tvState).thenReturn(RequestState.Loaded);
    when(mockNotifier.tv).thenReturn(testTvDetail);
    when(mockNotifier.recommendationState).thenReturn(RequestState.Loaded);
    when(mockNotifier.tvRecommendations).thenReturn(<Tv>[]);
    when(mockNotifier.isAddedToWatchlist).thenReturn(false);
    when(mockNotifier.watchlistMessage).thenReturn('Added to TV Watchlist');
    when(mockNotifier.tvSeasonState).thenReturn(RequestState.Loaded);
    when(mockNotifier.tvSeason).thenReturn(testTvSeason);
